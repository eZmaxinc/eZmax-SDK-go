...
> DomainGetAutocompleteV2Response DomainGetAutocompleteV2(ctx, sSelector).EFilterActive(eFilterActive).SQuery(sQuery).AcceptLanguage(acceptLanguage).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ObjectDomainAPI.DomainGetAutocompleteV2(context.Background(), sSelector).EFilterActive(eFilterActive).SQuery(sQuery).AcceptLanguage(acceptLanguage).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ObjectDomainAPI.DomainGetAutocompleteV2``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `DomainGetAutocompleteV2`: DomainGetAutocompleteV2Response
	fmt.Fprintf(os.Stdout, "Response from `ObjectDomainAPI.DomainGetAutocompleteV2`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiDomainGetAutocompleteV2Request struct via the builder pattern
